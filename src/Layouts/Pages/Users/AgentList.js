...
const AgentList = () => {
  ...
  const fetchAgentDetails = () => {
    dispatch(
      getUserDetails(loginDetails?.logindata?.Token, 5, (callback) => {
        if (callback.status) {
          setLoading(false);
          console.log(
          ...
          setAgent(callback?.response?.Details);
          setFilterAgentList(callback?.response?.Details);
        }
      })
    );
  };
